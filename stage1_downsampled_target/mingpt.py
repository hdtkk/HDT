...
from past_trans import Past_transformer, PastEmbedding
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size, block_size, enc_in, pred_len=24, codebook_embed=None, e_layers=3,
        ...
        self.tok_emb = nn.Embedding(config.vocab_size + 2, n_embd)
        self.tok_emb.weight.data.uniform_(-1.0 / config.vocab_size, 1.0 / config.vocab_size)

        self.pos_emb = nn.Parameter(torch.zeros(1, config.block_size, n_embd))  # 512 x 1024
        # self.pos_emb = PositionalEmbedding(d_model=n_embd, max_len=1000)
        ...
        self.head = nn.Sequential(nn.Linear(n_embd, n_embd),
                                  nn.ReLU(),
                                  nn.Linear(n_embd, config.vocab_size + 2, bias=True)
                                  )
    ...
    def forward(self, idx, x_past):
        ...
        # x_past_embed = torch.cat((sos_token, x_past_embed), dim=1)
        # print(x.shape)
        # past_x = x[:, :48, :]
        # future_x = x[:, 48:, :]
        # print(past_x.shape, x_past_embed.shape)
        # past_x = torch.cat((past_x, x_past_embed), dim=-1)
        # future_x = self.future_projection(future_x.permute(0, 2, 1)).permute(0, 2, 1)
        # x = torch.cat((x, x_past_embed), dim=-1)
        # print('x.shape',x.shape)
        # dsa
        # past_merge_embeddings = torch.cat((x_all[:, :past_embeddings.shape[1], :], past_embeds), dim=-1)

        # position_embeddings = self.pos_emb[:, :t, :]  # each position maps to a (learnable) vector
        # x = self.drop(token_embeddings + position_embeddings) # (B, 12, 128)
