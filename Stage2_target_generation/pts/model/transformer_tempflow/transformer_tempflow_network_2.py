...
class TransformerTempFlowTrainingNetwork(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.low_level_vqtransformer = VQGANTransformer(encoder=self.encoder, input_encoder=self.input_encoder,
        ...
        self.target_vqtransformer = Target_VQGANTransformer(encoder=self.stage1_encoder,
                                                     ...
                                                     pkeep=0.8, target_dim=1, e_layers=3, d_layers=3,
    ...
    def forward(
        ...
        past_forward_inputs = past_inputs[:, :self.prediction_length, :]
        past_back_inputs = past_inputs[:, self.prediction_length:, :]
        ...
        # logits, target = self.low_level_vqtransformer(x=mov_inputs_proj, x_past=past_back_indices, x_past_past=past_forward_indices)
        downsampled_indices = self.low_level_vqtransformer.sample_moving(x_past=past_back_indices, x_past_past=past_forward_indices)
        logits, target = self.target_vqtransformer(target_inputs_proj, past_back_indices, past_forward_indices, downsampled_indices)
