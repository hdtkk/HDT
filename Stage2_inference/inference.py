...
# trained_net = estimator.train(dataset_train, num_workers=0, stage_dict=state_dict, state_dict_stage1=state_dict_stage1)

# trained_net = estimator.train(dataset_train, num_workers=0, final_state=final_state_dict)
predictor = estimator.train(dataset_train, num_workers=0, final_state=final_state_dict)
...
# Compute qice lower is better
qice_l = []
for i in range(len(forecasts)):
    y_ture = targets[i][-96:].to_numpy()
    y_ture = y_ture.reshape(-1, 1)
    # print(y_ture.shape)
    y_pred = forecasts[i].samples
    y_pred = y_pred.reshape(-1, y_pred.shape[0])
    qice_score, _ =  qice(y_ture, y_pred)
    qice_l.append(qice_score)
qice =  sum(qice_l)
print('qice_sum', qice)
